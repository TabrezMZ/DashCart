class ProductManager {
  ...
  initModal() {
    ...
    photoUpload.addEventListener("click", () => {
      // Only open file dialog if image is not shown
      if (!this.tempImage) {
        photoInput.click();
      }
    });
    ...
    photoInput.addEventListener("change", (e) => {
      if (e.target.files && e.target.files[0]) {
        ...
        <div style="position: relative; width: 100%; height: 100px;">
          <img src="${ev.target.result}" alt="Product" style="width: 100%; height: 100%; object-fit: cover; border-radius: 8px;">
    ...
    companyDropdown.addEventListener("click", (e) => {
      ...
      if (option) {
        ...
        const companyOption = option
          .querySelector(".company-option")
          .cloneNode(true);

        companySelect
          .querySelector(".company-option")
          .replaceWith(companyOption);
  ...
  populateForm(product) {
    ...
    if (product.image) {
      photoUpload.innerHTML = `
  <div style="position: relative; width: 100%; height: 100px;">
    <img src="${product.image}" alt="Product" style="width: 100%; height: 100%; object-fit: cover; border-radius: 8px;">
    <button id="removeImageBtn" style="
      position: absolute;
      top: 6px;
      right: 6px;
...
`;
    ...
    companyOption.querySelector("span").textContent =
      product.company.charAt(0).toUpperCase() + product.company.slice(1);
    companyOption.querySelector(
      ".company-logo"
    ).className = `company-logo ${product.company}`;
  ...
  resetForm() {
    ...
    companyOption.querySelector(".company-logo").className =
      "company-logo google";
      ...
      .map(
        (product) => `
            <tr>
               ...
               <td>
                    <div class="actions">
                        <button class="action-btn" onclick="window.productManager.openModal(window.productManager.products.find(p => p.id === ${
                          product.id
                        }))">
                        ...
                        <button class="action-btn" onclick="window.productManager.deleteProduct(${
                          product.id
                        })">
                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
